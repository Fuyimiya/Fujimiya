...
export const sendLixeiraAlertaEmail = async (ecopontoNome: string, tipoLixeira: string, nivel: number) => {
    const adminEmail = process.env.ADMIN_EMAIL || ADMIN_CONTACT_EMAIL;

    await transporter.sendMail({
        from: `"EcoFluxo" <${EMAIL_USER}>`,
        to: adminEmail,
        subject: `Alerta Crítico: Ecoponto ${ecopontoNome}`,
        html: `
            <div style="font-family: sans-serif; padding: 20px; color: #333;">
                <h2 style="color: #dc2626;">Atenção, Gestor!</h2>
                <p>O sistema EcoFluxo detectou que um dos pontos de coleta atingiu o nível crítico de armazenamento.</p>
                <hr style="border: 0; border-top: 1px solid #eee;" />
                <p><strong>Local:</strong> ${ecopontoNome}</p>
                <p><strong>Lixeira afetada:</strong> Resíduo do tipo <b>${tipoLixeira}</b></p>
                <p><strong>Nível de Ocupação atual:</strong> <span style="color: #dc2626; font-weight: bold;">${nivel}%</span></p>
                <hr style="border: 0; border-top: 1px solid #eee;" />
                <p style="font-size: 12px; color: #777;">Este é um e-mail automático gerado pelo sistema de monitoramento do EcoFluxo.</p>
            </div>
        `,
    });
    console.log(`E-mail de alerta crítico enviado para: ${adminEmail}`);
};